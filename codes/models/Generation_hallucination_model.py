...
class GenerationModel(BaseModel):
    ...
    def test_crop2(self, input):
        img = input[0]
        mask = input[1]
        _, C, H, W = img.shape
        w_new = 2240
        imL = self.netG((img[:, :, :, 0:w_new], mask[:, :, :, 0:w_new]))[:, :, :, 0:int(W/2)]
        imR = self.netG((img[:, :, :, W-w_new:], mask[:, :, :, W-w_new:]))[:, :, :, w_new-int(W/2):]

        output_cat = torch.cat((imL, imR), 3)
